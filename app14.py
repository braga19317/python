...
# ---------- NOVA FUNÇÃO: SINAL DE TRADING ----------
def gerar_sinal_trading(dados_historicos, projecao_ensemble, tipo_analise):
    """
    Gera sinal de compra/venda baseado em RSI e tendência da projeção ensemble.
    Retorna sinal (string) e justificativa.
    """
    ultimo_rsi = dados_historicos['RSI'].iloc[-1] if 'RSI' in dados_historicos.columns else 50
    preco_atual = dados_historicos['Cotação'].iloc[-1]
    preco_projetado = projecao_ensemble[-1] if projecao_ensemble else preco_atual
    variacao_projetada = (preco_projetado / preco_atual - 1) * 100

    # Lógica de exemplo (ajuste os limiares conforme necessário)
    if ultimo_rsi < 30 and variacao_projetada > 1:
        sinal = "🟢 COMPRAR"
        justificativa = f"RSI em {ultimo_rsi:.1f} (sobrevendido) e projeção de alta de {variacao_projetada:.1f}%."
    elif ultimo_rsi > 70 and variacao_projetada < -1:
        sinal = "🔴 VENDER"
        justificativa = f"RSI em {ultimo_rsi:.1f} (sobrecomprado) e projeção de queda de {variacao_projetada:.1f}%."
    else:
        sinal = "⚪ NEUTRO"
        justificativa = "Mercado sem sinais claros no momento."

    return sinal, justificativa
...
# ---------- PDF EM BYTES ----------
def gerar_recomendacoes(metricas, dados_historicos, projecao_ensemble):
    recomendacoes = []
    rsi_atual = dados_historicos['RSI'].iloc[-1] if 'RSI' in dados_historicos.columns and not dados_historicos['RSI'].isna().all() else 50
    if rsi_atual > 70:
        recomendacoes.append("🔴 SINAL DE VENDA: RSI indica sobrecomprado (acima de 70).")
    elif rsi_atual < 30:
        recomendacoes.append("🟢 SINAL DE COMPRA: RSI indica sobrevendido (abaixo de 30).")
    else:
        recomendacoes.append("⚪ RSI em zona neutra.")

    if metricas.get('volatilidade', 0) > 15:
        recomendacoes.append("⚠️ ALTA VOLATILIDADE: risco elevado. Considere hedge.")
    else:
        recomendacoes.append("✅ Volatilidade moderada.")

    variacao_projetada = metricas.get('variacao_projetada', 0)
    if variacao_projetada > 5:
        recomendacoes.append(f"📈 TENDÊNCIA DE ALTA: projeção de +{variacao_projetada:.1f}%. Favorável para posições compradas em real.")
    elif variacao_projetada < -5:
        recomendacoes.append(f"📉 TENDÊNCIA DE BAIXA: projeção de {variacao_projetada:.1f}%. Favorável para posições compradas em moeda estrangeira.")
    else:
        recomendacoes.append(f"⚖️ TENDÊNCIA LATERAL: variação projetada de {variacao_projetada:.1f}%.")
...
"""
        recomendacoes = gerar_recomendacoes(metricas, dados_historicos, projecao_ensemble)
        relatorio_texto = f"""
...
def main():
    ...
    if 'analise_executada' in st.session_state and st.session_state['analise_executada']:
        ...
        sinal, justificativa = gerar_sinal_trading(res['dados_historicos'], res['projecao_ensemble'], res['tipo_analise'])
        st.subheader("📊 Sinal de Trading")
        col_sinal, col_just = st.columns([1, 3])
        with col_sinal:
            st.markdown(f"## {sinal}")
        with col_just:
        ...
        # --- NOVO: Simulação de Trade ---
        with st.expander("📈 Simular uma operação"):
            st.markdown("Defina os parâmetros da sua operação hipotética:")
            col_e1, col_e2, col_e3 = st.columns(3)
            with col_e1:
                preco_entrada = st.number_input("Preço de entrada (R$)", value=res['metricas']['valor_atual'], format="%.4f")
            with col_e2:
                stop_loss_pct = st.number_input("Stop Loss (%)", value=2.0, step=0.5) / 100
                stop_loss_preco = preco_entrada * (1 - stop_loss_pct)
                st.caption(f"Stop em R$ {stop_loss_preco:.4f}")
            with col_e3:
                take_profit_pct = st.number_input("Take Profit (%)", value=5.0, step=0.5) / 100
                take_profit_preco = preco_entrada * (1 + take_profit_pct)
                st.caption(f"Take em R$ {take_profit_preco:.4f}")

            # Verifica se a projeção final atinge os níveis
            projetado_final = res['projecao_ensemble'][-1]
            if projetado_final <= stop_loss_preco:
                st.error(f"❌ Projeção final ({projetado_final:.4f}) abaixo do stop loss ({stop_loss_preco:.4f}).")
            elif projetado_final >= take_profit_preco:
                st.success(f"✅ Projeção final ({projetado_final:.4f}) acima do take profit ({take_profit_preco:.4f}).")
            else:
                st.warning(f"⏳ Projeção final ({projetado_final:.4f}) entre stop e take.")
